# -*- coding: utf-8 -*-
# 计算所有的数据KM，那么就说明一个问题，数据分布直方图是计算准确的，然而文件名却对不上，说明数据生成是有问题的，要排除这个问题
import os
import tempfile
import numpy as np
import pandas as pd
...
OUTPUT_DIR_KM_DATA = r'D:\PINN\zenodo\POD_deeponet\data\km_data_for_test'
os.makedirs(OUTPUT_DIR_KM_DATA, exist_ok=True)  # Create output dir if needed

# Parameters for tau and A selection (UPDATED)
N_tau = 50  # Number of time delays (changed from 10 to 50)
TAU_RANGE = [0.01, 0.5]  # Tau selection range in seconds (new)
N_A = 80  # Number of amplitude points (changed from 49 to 80)
A_RANGE = [0, 8]  # Amplitude selection range (new)
...
def mirror_extension(signal, extension_len=500):
    """对信号进行镜像延拓以抑制边界效应"""
    if len(signal) < 2 * extension_len:
        # If signal is too short, just pad with its own values
        extension_len = len(signal) // 2
        if extension_len == 0: return signal  # Cannot extend a very short signal
...
def find_tau_A(autocorr, dt):
    ...
    (This function is now less critical for tau selection range, but can still provide insight)"""
    try:
        indices = np.where(autocorr <= 0.25)[0]
        if len(indices) > 0:
            first_index = indices[0]
            if first_index == 0 and len(indices) > 1:
...
def _compute_km_for_single_tau(tau_idx, A_original_local, A_selected_local, dt_local, A_min_data_kde, A_max_data_kde):
    """Helper function to compute KM coeffs for one tau_idx (for parallelization)."""
    if tau_idx >= len(
        A_original_local) or tau_idx <= 0: return tau_idx, None  # tau_idx must be positive and within bounds
    A_t = A_original_local[:-tau_idx]
    A_t_tau = A_original_local[tau_idx:]

    # Ensure A_t and A_t_tau have sufficient length
    if len(A_t) < 2 or len(A_t_tau) < 2: return tau_idx, None
    ...
    try:
        ...
        if np.all(A_t == A_t[0]) and np.all(A_t_tau == A_t_tau[0]):
            data_stack += np.random.normal(0, 1e-6 * (A_max_data_kde - A_min_data_kde + 1e-9), data_stack.shape)
    ...
    for A in A_selected_local:
        # Define y_grid based on global min/max of A_original for consistency
        # And ensure it covers the range around A
        y_range = A_max_data_kde - A_min_data_kde
        # Estimate local standard deviation for y_grid range
        # Consider a wider neighborhood for robust std estimation
        A_neighborhood_indices = np.where(np.abs(A_t - A) < y_range * 0.1)[0]
        if len(A_neighborhood_indices) > 1:
            y_std_est = np.std(A_t_tau[A_neighborhood_indices])
        else:
            y_std_est = y_range * 0.1  # Fallback if neighborhood is too small

        y_std_est = max(y_std_est, y_range * 0.01)  # Ensure a minimum range

        # Adjust y_grid to cover a reasonable range around A, clamped by overall data range
        y_grid_min = max(A_min_data_kde, A - 5 * y_std_est)
        y_grid_max = min(A_max_data_kde, A + 5 * y_std_est)

        # Ensure y_grid covers at least a small range if min/max are too close
        if y_grid_max - y_grid_min < 1e-9:
            y_grid_min = A - 0.1 * y_range
            y_grid_max = A + 0.1 * y_range
            # Re-clamp to overall data range
            y_grid_min = max(A_min_data_kde, y_grid_min)
            y_grid_max = min(A_max_data_kde, y_grid_max)
            if y_grid_max - y_grid_min < 1e-9:  # Still too small, use a default small range
                y_grid_min = A - 0.05
                y_grid_max = A + 0.05
...
def compute_finite_time_km_coefficients_and_save(file_path, N_tau_param, TAU_RANGE_param, N_A_param, A_RANGE_param,
                                                 F_FILTER_param, n_jobs_km, output_dir):
    """
    ...
    try:
        ...
        if 'Envelope' in df.columns:
            ...
            print(
                f"Error: Could not find a suitable column ('Envelope', 'eta_filtered', or 'Eta') in {original_file_name}. Skipping.")
            return False

        if A_original is None or len(A_original) < 1000:  # Basic check for sufficient data points
        ...
        if 'Time (s)' in df.columns:
            ...
            if dt <= 0:  # Handle cases where dt is not positive
                dt = 0.0001  # Fallback
        else:
            dt = 0.0001  # Fallback if time column is missing

        # Determine Time Delays (tau) based on fixed range (UPDATED)
        tau_min_sec = TAU_RANGE_param[0]
        tau_max_sec = TAU_RANGE_param[1]

        # Ensure tau_max_sec does not exceed the maximum possible tau from data length
        max_possible_tau_sec = (len(A_original) - 1) * dt
        if tau_max_sec > max_possible_tau_sec:
            # print(f"Warning: Specified tau_max ({tau_max_sec:.4f}s) exceeds data length ({max_possible_tau_sec:.4f}s) for {original_file_name}. Clamping tau_max.")
            tau_max_sec = max_possible_tau_sec * 0.9  # Use 90% of max possible to avoid issues

        if tau_min_sec >= tau_max_sec:
            # print(f"Warning: tau_min_sec ({tau_min_sec:.4f}s) is greater than or equal to adjusted tau_max_sec ({tau_max_sec:.4f}s) for {original_file_name}. Skipping KM calculation.")
            return False

        tau_values_sec = np.linspace(tau_min_sec, tau_max_sec, N_tau_param)
        tau_indices = (tau_values_sec / dt).astype(int)
        tau_indices = np.maximum(1, tau_indices)  # Ensure tau_idx is at least 1
        tau_indices = np.unique(tau_indices)

        # Filter out tau_indices that are too large for the data
        tau_indices = tau_indices[tau_indices < len(A_original)]
        if len(tau_indices) == 0:
            # print(f"Warning: No valid tau indices could be determined for {original_file_name}. Skipping KM calculation.")
            return False

        # Select Amplitude Points (A) based on fixed range (UPDATED)
        A_selected = np.linspace(A_RANGE_param[0], A_RANGE_param[1], N_A_param)

        A_min_data_kde, A_max_data_kde = np.min(A_original), np.max(A_original)
        if A_max_data_kde - A_min_data_kde < 1e-9:  # Handle cases with nearly constant amplitude
        ...
        results_list = Parallel(n_jobs=n_jobs_km, backend='loky')(
            delayed(_compute_km_for_single_tau)(tau_idx, A_original, A_selected, dt, A_min_data_kde, A_max_data_kde)
        ...
        for tau_idx, result_data in results_list:
            if result_data is not None:
                nan_frac_d1 = np.sum(np.isnan(result_data['D1'])) / len(result_data['D1']) if len(
                    result_data['D1']) > 0 else 1.0
                nan_frac_d2 = np.sum(np.isnan(result_data['D2'])) / len(result_data['D2']) if len(
                    result_data['D2']) > 0 else 1.0
...
if __name__ == '__main__':  # Ensure this block runs only when the script is executed directly
    ...
    results = Parallel(n_jobs=N_JOBS_FILE_PROCESSING, backend='loky')(
        delayed(compute_finite_time_km_coefficients_and_save)(
            file_path, N_tau, TAU_RANGE, N_A, A_RANGE, F_FILTER, N_JOBS_KM_CALCULATION, OUTPUT_DIR_KM_DATA
