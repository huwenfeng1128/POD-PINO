...
4. 输出：
   ...
   - Nelder-Mead / L-BFGS-B 两种优化方式的迭代时间对比
...
# --- 结果输出目录 ---
BASE_OUTPUT_DIR = r'D:\PINN\zenodo\POD_deeponet\PI-POD-DeepONet\SI_result_physics_informed'
...
# 主结果仍使用 Nelder-Mead，以保证原有绘图、KM 宽表和 result_summary
# 的最终参数定义不变；L-BFGS-B 仅作为第二种优化方式并行比较。
PRIMARY_OPTIMIZER_METHOD = 'Nelder-Mead'
...
OPTIMIZER_CONFIGS = {
    ...
    # 与示例表一致，L-BFGS-B 采用最多 100 次迭代。
    # 如需改成与 Nelder-Mead 相同的最大迭代次数，只需把 100 改为 3000。
    'L-BFGS-B': {
        'maxiter': 100,
        'ftol': 1e-12,
        'gtol': 1e-8,
...
# =============================================================================
# 5.1 两种优化器运行与计时（新增；目标函数本身保持不变）
# =============================================================================

def _sync_device_for_timing():
    """
    GPU 情况下同步 CUDA，避免异步执行导致计时偏小。
    CPU 情况下不做任何操作。
    """
    if DEVICE.type == 'cuda':
...
def run_optimizer_with_timing(method,
                              options,
                              params_0,
                              finite_time_km,
                              A_selected,
    ...
    其中：
    - Iteration 使用 scipy.optimize.minimize 返回的 result.nit；
    - Per Iteration (s) = Total (s) / result.nit；
    - optimization_history 仍由原 objective_function 记录，
      它统计的是目标函数调用次数，不用来冒充优化器迭代次数。
    """
    ...
    function_evaluations = int(getattr(result, 'nfev', len(optimization_history)) or 0)

    per_iteration_time = (
        total_time / optimizer_iterations
        if optimizer_iterations > 0
        else np.nan
    )

    # 与原代码保持同样的最优参数提取/回退规则
    if result.success:
        nu_opt, kappa_opt, d_opt = result.x
    else:
        print(f"  [{method}] Optimization did not converge. Using best from history.")

        if len(optimization_history) == 0:
            nu_opt, kappa_opt, d_opt = params_0
        else:
            valid_costs = np.asarray(
                [h['total_cost'] for h in optimization_history],
                dtype=float
            )
            valid_costs = np.where(
                np.isfinite(valid_costs),
                valid_costs,
                np.inf
            )
    ...
    # 方便另外保存两种优化器的参数结果，但不改变主结果选择。
    finite_costs = [
        float(h['total_cost'])
        for h in optimization_history
        if np.isfinite(h['total_cost'])
    ]
    best_cost = min(finite_costs) if finite_costs else np.nan

    timing_record = {
        'Method': f'PI-POD-DeepONet-{method}',
        'Per Iteration (s)': per_iteration_time,
        'Iteration': optimizer_iterations,
        'Total (s)': total_time
    }

    detail_record = {
        'Method': f'PI-POD-DeepONet-{method}',
        'nu_optimized': float(nu_opt),
        'kappa_optimized': float(kappa_opt),
        'd_diffusion_optimized': float(d_opt),
...
def process_km_file(km_file_path,
    ...
    # =========================================================================
    # 新增：分别运行 Nelder-Mead 和 L-BFGS-B，并记录迭代时间
    # 原目标函数、A 权重、误差归一化等全部不变。
    # =========================================================================
    optimizer_runs = {}

    for optimizer_method, optimizer_options in OPTIMIZER_CONFIGS.items():
        print(f"\n  >>> Running optimizer: {optimizer_method}")

        optimizer_runs[optimizer_method] = run_optimizer_with_timing(
        ...
        current_timing = optimizer_runs[optimizer_method]['timing']

        print(
            f"  [{optimizer_method}] "
            f"Iteration={current_timing['Iteration']}, "
            f"Per Iteration={current_timing['Per Iteration (s)']:.6f}s, "
            f"Total={current_timing['Total (s)']:.6f}s"
        )
    ...
    timing_df = pd.DataFrame(
        [
            optimizer_runs['Nelder-Mead']['timing'],
            optimizer_runs['L-BFGS-B']['timing']
        ],
        columns=[
            'Method',
            'Per Iteration (s)',
    ...
    timing_path = os.path.join(
        output_dir,
        'optimizer_timing_comparison.csv'
    )
    timing_df.to_csv(timing_path, index=False)

    print("\n  Optimizer timing comparison:")
    print(timing_df.to_string(index=False))
    print(f"  Timing table saved to: {timing_path}")

    # 另存两种优化器的参数/收敛细节，便于核对
    optimizer_detail_df = pd.DataFrame(
        [
            optimizer_runs['Nelder-Mead']['detail'],
            optimizer_runs['L-BFGS-B']['detail']
        ]
    )
    optimizer_detail_df.to_csv(
        os.path.join(output_dir, 'optimizer_result_comparison.csv'),
        index=False
    )

    # =========================================================================
    # 为保证“其他不变”，后续绘图、KM 宽表、result_summary
    # 仍使用原来的 Nelder-Mead 作为主优化结果。
    # =========================================================================
    primary_run = optimizer_runs[PRIMARY_OPTIMIZER_METHOD]

    result = primary_run['result']
    optimization_history = primary_run['history']
    nu_opt = primary_run['nu_opt']
    kappa_opt = primary_run['kappa_opt']
    d_opt = primary_run['d_opt']
    elapsed_time = primary_run['timing']['Total (s)']

    print(
        f"\n  Primary optimizer ({PRIMARY_OPTIMIZER_METHOD}) "
    ...
    # -------------------------------------------------------------------------
    # 保存当前参数组全部 KM 系数宽表
    # 仍使用主优化器 Nelder-Mead 的最终参数，不改变原有最终结果定义
    # -------------------------------------------------------------------------
    save_km_wide_table(
    ...
    # -------------------------------------------------------------------------
    # 保存优化历史
    # optimization_history.csv 保持原含义：主优化器 Nelder-Mead
    # 另外分别保存两种优化器的历史。
    # -------------------------------------------------------------------------
    if len(optimization_history) > 0:
        history_df = pd.DataFrame(optimization_history)
        history_df.to_csv(
            os.path.join(output_dir, 'optimization_history.csv'),
            index=False
        )

    for optimizer_method, run_info in optimizer_runs.items():
        method_history = run_info['history']
        ...
        if len(method_history) > 0:
            ...
            pd.DataFrame(method_history).to_csv(
                os.path.join(
                    output_dir,
                    f'optimization_history_{safe_method_name}.csv'
                ),
                index=False
            )
    ...
    result_summary = {
        ...
        # 两种优化器的时间指标
        'Nelder_Mead_per_iteration_s':
            optimizer_runs['Nelder-Mead']['timing']['Per Iteration (s)'],
        'Nelder_Mead_iteration':
            optimizer_runs['Nelder-Mead']['timing']['Iteration'],
        'Nelder_Mead_total_s':
            optimizer_runs['Nelder-Mead']['timing']['Total (s)'],

        'LBFGSB_per_iteration_s':
            optimizer_runs['L-BFGS-B']['timing']['Per Iteration (s)'],
        'LBFGSB_iteration':
            optimizer_runs['L-BFGS-B']['timing']['Iteration'],
        'LBFGSB_total_s':
            optimizer_runs['L-BFGS-B']['timing']['Total (s)']
    }
...
if __name__ == "__main__":
    ...
    if len(all_results) > 0:
        ...
        for _, row in summary_df.iterrows():
            timing_long_records.append({
                'filename': row['filename'],
                'Method': 'PI-POD-DeepONet-Nelder-Mead',
                'Per Iteration (s)': row['Nelder_Mead_per_iteration_s'],
                'Iteration': row['Nelder_Mead_iteration'],
                'Total (s)': row['Nelder_Mead_total_s']
            })

            timing_long_records.append({
                'filename': row['filename'],
                'Method': 'PI-POD-DeepONet-L-BFGS-B',
                'Per Iteration (s)': row['LBFGSB_per_iteration_s'],
                'Iteration': row['LBFGSB_iteration'],
                'Total (s)': row['LBFGSB_total_s']
            })

        batch_timing_df = pd.DataFrame(timing_long_records)
        batch_timing_path = os.path.join(
            BASE_OUTPUT_DIR,
            'batch_optimizer_timing_all.csv'
        )
        batch_timing_df.to_csv(batch_timing_path, index=False)

        # 对全部参数文件求平均，得到可直接用于论文表格的两行结果
        batch_timing_summary_df = (
            batch_timing_df
            .groupby('Method', as_index=False)
            .agg({
                'Per Iteration (s)': 'mean',
                'Iteration': 'mean',
                'Total (s)': 'mean'
            })
        )

        batch_timing_summary_path = os.path.join(
            BASE_OUTPUT_DIR,
            'batch_optimizer_timing_summary.csv'
        )
        batch_timing_summary_df.to_csv(
            batch_timing_summary_path,
            index=False
        )

        print("\n=== Mean Optimizer Timing Across All Files ===")
        print(batch_timing_summary_df.to_string(index=False))
        print(f"All timing records saved to: {batch_timing_path}")
        print(f"Mean timing summary saved to: {batch_timing_summary_path}")
    else:
