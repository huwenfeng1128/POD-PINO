...
B. with_pde_residual
   总损失 = 数据损失 + D2 非负约束 + PDE 物理残差

注意
----
- D2 非负约束在两个实验中始终保留，不对其做消融。
- 为保证模型选择标准公平，两个实验都使用：
      validation common objective
    = validation data loss + positivity weight * validation positivity loss
...
# lambda 扫描程序生成的最优权重选择文件。
# 可通过环境变量 OPTIMAL_LAMBDA_SELECTION_PATH 临时覆盖，无需修改代码。
OPTIMAL_LAMBDA_SELECTION_PATH = os.environ.get(
    "OPTIMAL_LAMBDA_SELECTION_PATH",
    r'D:\PINN\zenodo\POD_deeponet\PI-POD-DeepONet\result\Choice_lamda\train_result_lambda_sweep_2\pod_physics_lambda_sweep_normalized_residual_v2\optimal_lambda_selection.json',
)

RUN_ID = "pod_physics_residual_ablation_normalized_auto_lambda_v2"
...
# D2 非负约束在两个实验中始终存在，且不随 lambda 改变。
POSITIVITY_LOSS_WEIGHT = 0.1

# POD-PINO 的 lambda 从权重扫描结果 JSON 自动读取。
# lambda 只乘归一化 PDE 残差，不乘 D2 非负约束。
# 归一化残差：mean(R1^2 / S1) + mean(R2^2 / S2)，
# 其中 S1、S2 为各自控制方程组成项平方和。
...
def load_optimal_lambda(selection_path: str) -> Tuple[float, Dict]:
    """从 lambda 扫描结果读取 optimal_lambda，并进行严格校验。"""
    if not os.path.isfile(selection_path):
        raise FileNotFoundError(
            "未找到最优 lambda 选择文件：\n"
            f"{selection_path}\n"
            "请修改 OPTIMAL_LAMBDA_SELECTION_PATH，或设置同名环境变量。"
        )
    with open(selection_path, "r", encoding="utf-8") as f:
        payload = json.load(f)
    if "optimal_lambda" not in payload:
        raise KeyError(f"文件 {selection_path} 中缺少 optimal_lambda 字段。")
    value = float(payload["optimal_lambda"])
    if not np.isfinite(value) or value < 0.0:
        raise ValueError(f"optimal_lambda 必须为非负有限数，实际为 {value!r}。")
...
class PhysicsInformedLoss(nn.Module):
    """统一计算数据项、固定 D2 非负项和归一化 PDE 残差项。"""

    def __init__(self, unified_a_grid, unified_tau_grid, branch_mean, branch_std,
                 y_mean_scaler, y_std_scaler, d2_data_weight, positivity_weight,
                 physics_weight, normalized_residual_eps):
        super().__init__()
        ...
        self.normalized_residual_eps = float(normalized_residual_eps)
        self.mse = nn.MSELoss()
        ...
        self.register_buffer("a_grid", torch.tensor(unified_a_grid, dtype=DTYPE))
        self.register_buffer("tau_grid", torch.tensor(unified_tau_grid, dtype=DTYPE))
        self.register_buffer("branch_mean", torch.tensor(branch_mean, dtype=DTYPE))
        self.register_buffer("branch_std", torch.tensor(branch_std, dtype=DTYPE))
        self.register_buffer("y_mean_scaler", torch.tensor(y_mean_scaler, dtype=DTYPE))
        self.register_buffer("y_std_scaler", torch.tensor(y_std_scaler, dtype=DTYPE))
        self.da = float(unified_a_grid[1] - unified_a_grid[0])
        self.dtau = float(unified_tau_grid[1] - unified_tau_grid[0])

    @property
    def effective_pde_coefficient(self):
    ...
    def compute_residual_maps(self, y_pred_scaled, branch_x_scaled):
        ...
        r1_t4 = d1_th_in.expand_as(r1_t1)
        r1 = r1_t1 - r1_t2 - r1_t3 - r1_t4
        r1_scale_sq = r1_t1.square()+r1_t2.square()+r1_t3.square()+r1_t4.square()
        r2_t1 = du2_dtau
        r2_t2 = d1_th_in * du2_da
        r2_t3 = d2_th_in * d2u2_da2
        r2_t4 = d1_th_in * u1_in
        r2_t5 = 2.0 * d2_th_in * du1_da
        r2_t6 = d2_th_in.expand_as(r2_t1)
        r2 = r2_t1-r2_t2-r2_t3-r2_t4-r2_t5-r2_t6
        r2_scale_sq = (r2_t1.square()+r2_t2.square()+r2_t3.square()+r2_t4.square()+r2_t5.square()+r2_t6.square())
        diagnostics = {
            "r1_scale_sq": r1_scale_sq, "r2_scale_sq": r2_scale_sq,
            "du1_dtau": du1_dtau, "du1_da": du1_da, "d2u1_da2": d2u1_da2,
            "du2_dtau": du2_dtau, "du2_da": du2_da, "d2u2_da2": d2u2_da2,
        }
        return r1, r2, diagnostics

    def compute_components(self, y_pred_scaled, y_true_scaled, branch_x_scaled):
        field_len = self.num_a * self.num_tau
        ...
        data_loss = (1.0-self.d2_data_weight)*loss_d1 + self.d2_data_weight*loss_d2
        y_pred = y_pred_scaled * self.y_std_scaler + self.y_mean_scaler
        d2_pred = y_pred[:, field_len:].view(-1, self.num_tau, self.num_a)
        positivity_loss = torch.mean(torch.relu(-d2_pred).square())
        r1, r2, diag = self.compute_residual_maps(y_pred_scaled, branch_x_scaled)
        pde_r1_raw = torch.mean(r1.square())
        pde_r2_raw = torch.mean(r2.square())
        pde_raw = pde_r1_raw + pde_r2_raw
        pde_r1_norm = torch.mean(r1.square() / (diag["r1_scale_sq"] + self.normalized_residual_eps))
        pde_r2_norm = torch.mean(r2.square() / (diag["r2_scale_sq"] + self.normalized_residual_eps))
        pde_norm = pde_r1_norm + pde_r2_norm
        common = data_loss + self.positivity_weight * positivity_loss
        weighted_pde = self.physics_weight * pde_norm
        return {"data": data_loss, "d1": loss_d1, "d2": loss_d2, "pos": positivity_loss,
                "pde": pde_norm, "pde_r1": pde_r1_norm, "pde_r2": pde_r2_norm,
                "pde_raw": pde_raw, "pde_r1_raw": pde_r1_raw, "pde_r2_raw": pde_r2_raw,
                "common": common, "weighted_pde": weighted_pde}

    def total_loss(self, components, use_pde):
        return components["common"] + components["weighted_pde"] if use_pde else components["common"]
...
def build_loss(shared: SharedData) -> PhysicsInformedLoss:
    return PhysicsInformedLoss(
        ...
        positivity_weight=POSITIVITY_LOSS_WEIGHT,
...
def append_train_log(
    ...
    logs["train_pos"].append(float(comp["pos"].detach().cpu()))
    ...
    logs["train_pde_raw"].append(float(comp["pde_raw"].detach().cpu()))
    logs["train_pde_r1_raw"].append(float(comp["pde_r1_raw"].detach().cpu()))
    logs["train_pde_r2_raw"].append(float(comp["pde_r2_raw"].detach().cpu()))
...
def compute_gradient_diagnostics(
    ...
    v_pos = gradients_to_vector(g_pos, params)
    ...
    pos_norm = torch.linalg.vector_norm(v_pos)
    ...
    result = {
        ...
        "positivity_grad_norm": float(pos_norm.detach().cpu()),
        ...
        "diagnostic_pos_loss": float(comp["pos"].detach().cpu()),
        ...
        records.append(
            {
                ...
                "d2_negative_fraction": np.mean(d2_pred < 0.0),
...
def evaluate_predictions(
    ...
    metrics["d2_negative_magnitude_mse"] = float(
        np.mean(np.maximum(-d2_pred, 0.0) ** 2)
    )
...
def finalize_experiment(
    ...
    metrics["effective_pde_coefficient"] = (
        float(PHYSICS_LOSS_WEIGHT) if use_pde else 0.0
    )
...
def plot_loss_ablation(
    ...
    axes[1, 1].set_title("Validation normalized PDE residual loss")
    ...
    fig.suptitle("Normalized physics-residual ablation: loss comparison", fontsize=17)
...
def plot_metric_ablation(
    ...
    # 非负约束只展示，不作为消融变量。
    pos_keys = ["d2_negative_fraction", "d2_negative_magnitude_mse"]
    pos_labels = ["Negative fraction", "Negative magnitude MSE"]
    x3 = np.arange(len(pos_keys))
    axes[2].bar(
        x3 - width / 2,
        [max(base.metrics[k], 1e-30) for k in pos_keys],
        width,
        label="Without PDE",
    )
    axes[2].bar(
        x3 + width / 2,
        [max(phys.metrics[k], 1e-30) for k in pos_keys],
        width,
        label="With PDE",
    )
    axes[2].set_xticks(x3)
    axes[2].set_xticklabels(pos_labels, rotation=20, ha="right")
    axes[2].set_yscale("log")
    axes[2].set_title("D2 positivity diagnostics")
    axes[2].grid(True, axis="y", which="both", alpha=0.3)
    axes[2].legend()
...
def build_ablation_report(
    ...
    report = f"""POD-DeepONet / POD-PINO 归一化物理残差消融实验报告
{'=' * 64}
...
- 两组最佳模型均按 common objective=data loss+positivity loss 选择。

2. POD 与权重信息
- 自动读取的最优 lambda: {float(PHYSICS_LOSS_WEIGHT):.8e}
- 归一化残差 epsilon: {NORMALIZED_RESIDUAL_EPS:.3e}
...
6. 非负约束诊断（不是本次消融变量）
- D2 negative fraction, no PDE={base.metrics['d2_negative_fraction']:.8e}
- D2 negative fraction, with PDE={phys.metrics['d2_negative_fraction']:.8e}
...
def main() -> None:
    global PHYSICS_LOSS_WEIGHT
    PHYSICS_LOSS_WEIGHT, lambda_selection_info = load_optimal_lambda(OPTIMAL_LAMBDA_SELECTION_PATH)
    print("=" * 96)
    print("POD-DeepONet / POD-PINO Normalized Physics Residual Ablation")
    print("=" * 96)
    print(f"Device     : {DEVICE}")
    print(f"Dtype      : {DTYPE}")
    print(f"Result dir : {RESULT_DIR}")
    print(f"Data dir   : {DATA_DIR}")
    print(f"Lambda JSON: {OPTIMAL_LAMBDA_SELECTION_PATH}")
    print(f"Optimal lambda: {PHYSICS_LOSS_WEIGHT:.12g}")
    ...
    config = {
        ...
        "POSITIVITY_LOSS_WEIGHT": POSITIVITY_LOSS_WEIGHT,
        "OPTIMAL_LAMBDA_SELECTION_PATH": OPTIMAL_LAMBDA_SELECTION_PATH,
        "PHYSICS_LOSS_WEIGHT": float(PHYSICS_LOSS_WEIGHT),
        "NORMALIZED_RESIDUAL_EPS": NORMALIZED_RESIDUAL_EPS,
        "EFFECTIVE_PDE_COEFFICIENT": float(PHYSICS_LOSS_WEIGHT),
    ...
    save_json(os.path.join(RESULT_DIR, "optimal_lambda_selection_used.json"), lambda_selection_info)
