...
# PDE 原始残差先乘 PDE_LOSS_SCALE，再乘 PHYSICS_LOSS_WEIGHT。
# 与原始代码 0.1 * 0.01 * loss_pde 保持一致。
...
class PhysicsInformedLoss(nn.Module):
    ...
    def __init__(
        ...
        self.pde_loss_scale = float(pde_loss_scale)
        self.mse = nn.MSELoss()
    ...
    def effective_pde_coefficient(self) -> float:
        return self.physics_weight * self.pde_loss_scale
    ...
    def compute_components(
        ...
        y_pred = y_pred_scaled * self.y_std_scaler + self.y_mean_scaler
        d2_pred = y_pred[:, field_len:].view(-1, self.num_tau, self.num_a)
        positivity_loss = torch.mean(torch.relu(-d2_pred) ** 2)
        ...
        common_objective = data_loss + self.positivity_weight * positivity_loss
...
def finalize_experiment(
    ...
    metrics["effective_pde_coefficient"] = (
        PHYSICS_LOSS_WEIGHT * PDE_LOSS_SCALE if use_pde else 0.0
    )
...
def main() -> None:
    ...
    config = {
        ...
        "PDE_LOSS_SCALE": PDE_LOSS_SCALE,
        "EFFECTIVE_PDE_COEFFICIENT": PHYSICS_LOSS_WEIGHT * PDE_LOSS_SCALE,
