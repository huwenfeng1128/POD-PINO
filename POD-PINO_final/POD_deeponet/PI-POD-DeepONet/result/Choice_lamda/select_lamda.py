...
本程序用于系统分析物理损失权重 lambda 对模型精度与物理一致性的影响。
...
lambda 的定义
-------------
    L_total = L_data + lambda * (L_pos + PDE_LOSS_SCALE * L_pde)
...
BASE_RESULT_DIR = (
    r'D:\PINN\zenodo\POD_deeponet\PI-POD-DeepONet\result\Choice_lamda\train_result_lambda_sweep'
)
...
RUN_ID = 'pod_physics_lambda_sweep_v1'
...
#     data_loss
#     +
#     lambda * (positivity_loss + PDE_LOSS_SCALE * pde_loss)
#
# 建议：
# 1. 首先使用下面的对数网格进行粗搜索；
# 2. 找到较优范围后，再在较优 lambda 附近增加更密集的取值。
#
LAMBDA_VALUES = [
    0.0,
    1.0e-4,
...
# PDE 残差通常比非负约束的数值大，因此保留一个固定缩放因子。
#
# lambda 控制整体物理损失权重；
# PDE_LOSS_SCALE 控制物理损失内部 PDE 残差和非负损失之间的比例。
PDE_LOSS_SCALE = 0.01
...
def plot_single_lambda_physics(
    ...
    ax.set_title(
        f'Validation physics metrics, lambda={lambda_phys:g}'
    )
