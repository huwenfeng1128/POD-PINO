...
4. 每个样本保存：
   - 参照标准网格场结构导出的统一 CSV (field_comparison_data_*.csv)，
     包含 A, Tau 网格、真值二维插值(D1/D2_True_Interp)、双模型预测场、
     绝对误差场、双模型差值场，以及全量 R1、R2 残差场（R1/R2_no_pde_residual, R1/R2_with_pde_residual, R1/R2_Diff），
...
# 前向预测输出目录
OUTPUT_DIR = Path(
    r"D:\PINN\zenodo\POD_deeponet\PI-POD-DeepONet\result\forward_predict\forward_prediction_physics_ablation_4"
)
...
def calculate_sample_metrics(
    ...
    for model_name in MODEL_NAMES:
        ...
        row["pde_r2_rmse"] = float(np.sqrt(row["pde_r2_mse"]))
...
def full_grid_long_table(
    ...
    """生成包含网格坐标 A, Tau、插值真值、双模型预测场、绝对误差场、模型差值场，
    以及全量 R1、R2 残差场的标准化 CSV。
    ...
    # 4. 计算并完整保存 R1、R2 PDE 残差场矩阵 (边界剥离部分自动填充 NaN)
    for model_name in MODEL_NAMES:
        r1, r2, _ = compute_residual_maps(
            full_predictions[model_name]["D1"][0],
            full_predictions[model_name]["D2"][0],
            params, a_grid, tau_grid
        )
        r1_full = np.full((len(tau_grid), len(a_grid)), np.nan)
        r2_full = np.full((len(tau_grid), len(a_grid)), np.nan)
        r1_full[1:-1, 1:-1] = r1
        r2_full[1:-1, 1:-1] = r2

        df[f"R1_{model_name}"] = r1_full.ravel()
        df[f"R2_{model_name}"] = r2_full.ravel()

    df["R1_Diff_with_minus_without"] = df["R1_with_pde_residual"] - df["R1_no_pde_residual"]
    df["R2_Diff_with_minus_without"] = df["R2_with_pde_residual"] - df["R2_no_pde_residual"]
...
def process_one_sample(
    ...
    # 按照标准化网格保存平坦化的 2D 场数据 CSV (包含 D1, D2 预测场、真值场、误差场及全量 R1, R2 残差场)
...
def build_analysis_report(
    ...
    lines.extend([
        ...
        "",
        "6. 建议优先检查的输出",
        "- field_data_samples/field_comparison_data_*.csv：标准二维网格插值场数据，包含全量 R1, R2 残差，极利于 Origin 绘图。",
