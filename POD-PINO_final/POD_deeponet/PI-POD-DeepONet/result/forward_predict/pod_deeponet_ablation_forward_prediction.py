# -*- coding: utf-8 -*-
"""
POD-DeepONet / POD-PINO 双模型前向预测与消融对比分析
====================================================
...
# 前向预测输出目录
OUTPUT_DIR = Path(
    r"D:\PINN\zenodo\POD_deeponet\PI-POD-DeepONet\result\forward_predict\forward_prediction_physics_ablation_2"
)
...
def full_grid_long_table(
    ...
    for model_name in MODEL_NAMES:
        df[f"D1_{model_name}"] = full_predictions[model_name]["D1"][0].ravel()
        df[f"D2_{model_name}"] = full_predictions[model_name]["D2"][0].ravel()

    df["D1_with_minus_without"] = (
        df["D1_with_pde_residual"] - df["D1_no_pde_residual"]
    )
    df["D2_with_minus_without"] = (
        df["D2_with_pde_residual"] - df["D2_no_pde_residual"]
    )
    return df
...
def pivot_field(point_df: pd.DataFrame, value_col: str):
    pivot = point_df.pivot_table(
        index="tau", columns="A", values=value_col, aggfunc="mean"
...
def plot_field_comparison(
    ...
    a, tau, true_z = pivot_field(point_df, true_col)
    _, _, no_z = pivot_field(point_df, no_col)
    _, _, with_z = pivot_field(point_df, with_col)
    no_err = no_z - true_z
...
def process_one_sample(
    ...
    full_grid_df.to_csv(
        data_dir / "full_model_grid_predictions.csv",
        index=False,
        encoding="utf-8-sig",
    )
...
def build_analysis_report(
    ...
    lines = [
        "POD-DeepONet / POD-PINO 双模型前向预测分析报告",
        "=" * 70,
        "",
        "1. 运行概况",
...
def main() -> None:
    ...
    print("=" * 88)
    print("POD-DeepONet / POD-PINO 双模型前向预测")
