# -*- coding: utf-8 -*-
"""
POD-DeepONet / POD-PINO 双模型前向预测 V2与消融对比分析
====================================================
...
def pivot_field(
    ...
    pandas.pivot_table 会自动删除“整行全为 NaN”的 tau 层，这正是旧版出现
    (9,49) 与 (10,49) 无法相减的根因。V2 先从坐标列建立公共 A/tau 网格，
    再对每个字段显式 reindex，因此真值缺失层仍会保留为 NaN，预测场、真值场
    和误差场始终具有完全相同的形状。
    """
    if a_values is None:
...
def plot_field_comparison(
    ...
    # 所有字段共享同一套坐标，缺失真值层保留为 NaN，不会改变数组形状。
    a_values = np.sort(point_df["A"].dropna().unique().astype(float))
    tau_values = np.sort(point_df["tau"].dropna().unique().astype(float))
...
def process_one_sample(
    ...
    # V2：显式保存真值与预测有效性掩膜。所有指标仅使用对应字段的有效点。
