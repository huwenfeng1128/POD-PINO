...
BASE_OUTPUT_DIR = r'D:\PINN\zenodo\POD_deeponet\compare\result\Out_POD-DeepONet_result_2'
...
def process_km_file(km_file_path, deeponet_model, scalers, unified_a_grid, unified_tau_grid,
    ...
    if os.path.exists(sim_file_path):
        try:
            ...
            sim_envelope = (np.abs(hilbert(df_sim['Eta'].values)) if 'Eta' in df_sim.columns
                            else df_sim['Envelope'].values)
...
if __name__ == "__main__":
    print("\n=== Loading Pre-trained DeepONet V3 Model ===")
    ...
    try:
        ...
        print(f"CRITICAL ERROR: Failed to load DeepONet model.")
        print(f"Error: {e}")
        traceback.print_exc()
        exit(1)
    ...
    if all_results:
        ...
        summary_path = os.path.join(BASE_OUTPUT_DIR, "batch_summary_v3.csv")
