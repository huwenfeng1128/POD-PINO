...
from scipy.interpolate import interp1d, griddata  # 引入 griddata
    ...
    try:
        ...
        if return_fields:
            field_data = {
                ...
                'params': true_params,
                # 存成 dict 形式
                'true_data_df': data_km_df[['tau_sec', 'A', 'D1_data', 'D2_data']].to_dict('list')
            }
...
def plot_field_comparison(field_data_size_min, field_data_size_max, filename, output_dir):
    """
    绘制场对比图 - 修复了 DataFrame 读取问题
    """
    # 1. 提取数据 (使用 .item() 解决 0-d array 问题)
    try:
        # 检查是否为 0-d array (即被 wrap 过的 dict)
        if field_data_size_max['true_data_df'].ndim == 0:
            true_data_dict = field_data_size_max['true_data_df'].item()
        else:
            true_data_dict = field_data_size_max['true_data_df']

        true_df = pd.DataFrame(true_data_dict)
    except Exception as e:
        print(f"Error extracting true dataframe: {e}")
        return
    ...
    # 使用 linear 插值，可能会有 NaN (在凸包外部)，fill_value 处理
...
if __name__ == "__main__":
        ...
        for filename in tqdm(test_files, desc=f"Size {data_size}"):
            ...
            if metrics:
                ...
                if is_vis_sample and field_data:
                    field_data['data_size'] = data_size
                    save_path = os.path.join(DIR_FIELDS, f'field_{filename[:-4]}_size_{data_size}.npz')
                    np.savez(save_path, **field_data)
    ...
    # 4. Field Visualization (Fixed Loading)
    ...
    for vis_file in vis_files:
        try:
            path_min = os.path.join(DIR_FIELDS, f'field_{vis_file[:-4]}_size_{min_size}.npz')
            path_max = os.path.join(DIR_FIELDS, f'field_{vis_file[:-4]}_size_{max_size}.npz')

            if os.path.exists(path_min) and os.path.exists(path_max):
                data_min = np.load(path_min, allow_pickle=True)
                data_max = np.load(path_max, allow_pickle=True)

                plot_field_comparison(data_min, data_max, vis_file, DIR_PLOTS)
