...
import random # 导入 random 模块用于文件抽样

# --- Configuration ---
# Directories and Files
DATA_DIR = r'D:\PINN\zenodo\AFP\DeepONet_Data_DynamicA_DynN\Train_data_2'  # ! 指向您生成数据的目录 !
MODEL_SAVE_PATH = r'D:\PINN\zenodo\AFP\result\Loss_data\model_tset_1000.pth'  # 更新模型保存路径
SCALER_SAVE_PATH = r'D:\PINN\zenodo\AFP\result\Loss_data\scalers_test_1000.pth'
PLOT_SAVE_PATH = r'D:\PINN\zenodo\AFP\result\Loss_data\training_loss_tset_1000.png'  # 更新绘图保存路径
LOSS_DATA_SAVE_PATH = r'D:\PINN\zenodo\AFP\result\Loss_data\loss_data_tset_1000.csv' # 新增：损失数据保存路径

# Model Hyperparameters
BRANCH_INPUT_DIM = 3   # nu, kappa, d_diffusion
...
N_BRANCH_EVAL_POINTS = 21 # 保持不变，这是数据生成时的点数
ADAM_ITERATIONS = 500000 # 适当调整迭代次数，根据实际数据量和收敛情况
LBFGS_ITERATIONS = 0     # LBFGS 迭代次数设置为 0
LOSS_SWITCH_ITER = 15000 # 相应调整损失函数切换的迭代次数
ADAM_LR = 0.001
LR_DECAY_RATE = 0.98
LR_DECAY_STEP = 1000     # 保持与原版相近，或根据需要调整
ADAM_BATCH_SIZE = 1024   # 保持与原版相近，或根据需要调整
LBFGS_BATCH_SIZE = 2**16 # 即使 LBFGS_ITERATIONS=0，也保留此参数
VALIDATION_SPLIT = 0.2
SEED = 42
VALIDATION_FREQUENCY = 100 # 每 N 次迭代计算一次验证损失
# --- 新增: 早停配置 ---
EARLY_STOPPING_THRESHOLD = 1e-6 # 验证损失低于此阈值时停止 (这个阈值可能需要根据实际情况调整)
EARLY_STOPPING_PATIENCE = 1000    # 连续 N 次验证损失低于阈值才停止 (可选, 增加稳定性)

# Data Sampling Configuration (针对文件数量)
TARGET_NUM_FILES =1000 # 目标训练文件数量
...
# --- Helper Functions ---
def load_and_preprocess_data(data_dir, target_num_files, validation_split, seed):
    """
    ...
    df_list = []
    for f in tqdm(selected_files, desc="读取 CSV 文件"):
        try:
    ...
    print(f"加载了 {len(full_df)} 个数据点。") # 这是加载的实际数据点总数

    initial_len = len(full_df)
    full_df.dropna(subset=['D1_fp', 'D2_fp'], inplace=True)
    cleaned_len = len(full_df)
    print(f"清除了 {initial_len - cleaned_len} 行含有 NaN 目标值的数据。剩余 {cleaned_len} 个数据点。")
    if cleaned_len == 0:
        raise ValueError("所有数据行都包含 NaN 目标值，无法继续。")

    branch_inputs = full_df[['nu', 'kappa', 'd_diffusion']].values
    trunk_inputs = full_df[['A', 'tau']].values
    outputs = full_df[['D1_fp', 'D2_fp']].values

    # 这里的 train_test_split 仍然是基于加载进来的所有数据点进行的
    (branch_train, branch_val,
     trunk_train, trunk_val,
     y_train, y_val) = train_test_split(branch_inputs, trunk_inputs, outputs,
...
def manual_scaler(data, mean=None, std=None):
    ...
    if mean is None or std is None:
        ...
        # 确保 std 不为零
        std_tensor = torch.as_tensor(std) if isinstance(std, np.ndarray) else std
        std_tensor[std_tensor < 1e-10] = 1.0
        scaled_data = (data - mean) / std_tensor.numpy() if isinstance(std, np.ndarray) else (data - mean) / std_tensor
        return scaled_data
...
if __name__ == "__main__":
    ...
    try:
        branch_train_np, branch_val_np, trunk_train_np, trunk_val_np, y_train_np, y_val_np = \
            load_and_preprocess_data(DATA_DIR, TARGET_NUM_FILES, VALIDATION_SPLIT, SEED)
    ...
    while global_iter < ADAM_ITERATIONS and not early_stopping_triggered:
        ...
        for batch_branch, batch_trunk, batch_y in train_loader_adam:
            ...
            if global_iter % VALIDATION_FREQUENCY == 0:
                ...
                print(f"Iter: {global_iter}, Train Loss ({loss_type}): {loss.item():.6e}, Val Loss: {avg_val_loss:.6e}, LR: {scheduler.get_last_lr()[0]:.6e}")
                ...
                model.train()
                 ...
                 loss_type = "Phase 1" if global_iter < LOSS_SWITCH_ITER else "Phase 2"
                 print(f"Iter: {global_iter}, Train Loss ({loss_type}): {loss.item():.6e}, LR: {scheduler.get_last_lr()[0]:.6e}")
    ...
    if LBFGS_ITERATIONS > 0 and not early_stopping_triggered:
        ...
        while global_iter < ADAM_ITERATIONS + LBFGS_ITERATIONS and not early_stopping_triggered:
            model.train()
            for batch_branch, batch_trunk, batch_y in DataLoader(train_dataset, batch_size=min(LBFGS_BATCH_SIZE, len(train_dataset)), shuffle=True):
                 ...
                 if global_iter % VALIDATION_FREQUENCY == 0 or global_iter == lbfgs_start_iter:
                    ...
                    val_loss_log.append(avg_val_loss)
                    loss_type = "Phase 1" if global_iter < LOSS_SWITCH_ITER else "Phase 2"
                    print(f"Iter: {global_iter}, Train Loss ({loss_type}): {current_loss:.6e}, Val Loss: {avg_val_loss:.6e}")
    ...
    if val_iterations_log:
        plt.plot(val_iterations_log, val_loss_log, label='Validation Loss', alpha=0.8, linewidth=1.5, linestyle='-', marker='o', markersize=3)

    actual_adam_end_iter = min(ADAM_ITERATIONS, total_iterations)
    if ADAM_ITERATIONS > 0 and actual_adam_end_iter > 0:
         plt.axvline(x=actual_adam_end_iter, color='gray', linestyle='--', label=f'Adam End (Iter {actual_adam_end_iter})')

    actual_loss_switch_iter = min(LOSS_SWITCH_ITER, total_iterations)
    if LOSS_SWITCH_ITER > 0 and actual_loss_switch_iter > 0:
        plt.axvline(x=actual_loss_switch_iter, color='red', linestyle='--', label=f'Loss Switch (Iter {actual_loss_switch_iter})')

    if early_stopping_triggered and val_iterations_log:
        stop_iter = val_iterations_log[-1]
        stop_loss = val_loss_log[-1]
        plt.scatter([stop_iter], [stop_loss], color='red', s=100, zorder=5, label=f'Early Stop Trigger ({stop_iter}, {stop_loss:.2e})')


    plt.xlabel('Iteration')
    plt.ylabel('Loss')
    plt.yscale('log')
    plt.title('Training and Validation Loss Curve with Early Stopping (2000 Sampled Files)') # 更新标题
    plt.legend()
    ...
    # 9. 保存损失数据到 CSV 文件
    print("保存损失数据到 CSV 文件...")
    # 确保 val_iterations_log 和 val_loss_log 与 iterations_log 和 loss_log 长度匹配，
    # 或者创建 DataFrame 时进行适当的合并。
    # 这里我们创建一个包含所有训练迭代的DataFrame，并填充验证损失，未记录的验证迭代则为NaN。
    loss_df = pd.DataFrame({
        'Iteration': iterations_log,
        'Training_Loss': loss_log
    })

    # 创建一个验证损失的DataFrame
    val_loss_df = pd.DataFrame({
        'Iteration': val_iterations_log,
        'Validation_Loss': val_loss_log
    })

    # 将两个DataFrame合并，基于 'Iteration' 列
    # 使用 outer merge 以保留所有迭代，未匹配的会是 NaN
    full_loss_df = pd.merge(loss_df, val_loss_df, on='Iteration', how='outer')
    full_loss_df.sort_values(by='Iteration', inplace=True) # 确保按迭代次数排序

    os.makedirs(os.path.dirname(LOSS_DATA_SAVE_PATH), exist_ok=True)
    try:
        full_loss_df.to_csv(LOSS_DATA_SAVE_PATH, index=False)
        print(f"损失数据已保存到 {LOSS_DATA_SAVE_PATH}")
    except Exception as e:
        print(f"保存损失数据到 CSV 文件时出错: {e}")